...
# note that data strings are defined in the module cfour_dat.py
...
#==========================================================================
# a function to establish data structures for cfour methods
# called once on startup/restart
#==========================================================================

def setup(s):
  ' Function to setup for cfour calculations '
  print '    cfour setup \n' 

  # interrogate the dictionary cfour_dat.scftype
  method=s.AI_Simple
  scftype = ''
  for k in cfour_dat.scftype:
    if cfour_dat.scftype[k].count(method) == 1:
      scftype = k
      break
  ...
  if scftype == 'rscf':
     s.AI_Command['scf'] = []
     for i in range(0,len(cfour_dat.rscf)):
	cmd=cfour_dat.rscf[i]
	...
	cmd+='MULT=' + str(s.AI_Spin) + "," + '\n'
     ...
     s.AI_Command['scf'] = []
     for i in range(0,len(cfour_dat.uscf)):
	cmd=cfour_dat.uscf[i]
	cmd+='MEMORY_SIZE=' + s.AI_Memory + "," + '\n'
	cmd+='CHARGE=' + str(s.AI_Charge) + "," + '\n'
	cmd+='MULT=' + str(s.AI_Spin) + "," + '\n'
        cmd+='BASIS=' + s.AI_Basis + ")" + '\n\n'
     ...
     s.AI_Command['scf'] = []
     for i in range(0,len(cfour_dat.roscf)):
	cmd=cfour_dat.roscf[i]
	cmd+='MEMORY_SIZE=' + s.AI_Memory + "," + '\n'
	cmd+='CHARGE=' + str(s.AI_Charge) + "," + '\n'
	cmd+='MULT=' + str(s.AI_Spin) + "," + '\n'
        cmd+='BASIS=' + s.AI_Basis + ")" + '\n\n'
  ...
  if s.AI_JobType == 'simple':
   ...
   # first the High Accuracy calculation at Small basis
    cmd=cfour_dat.methods[s.AI_High]
    cmd+='MEMORY_SIZE=' + s.AI_Memory + "," + '\n'
    cmd+='CHARGE=' + str(s.AI_Charge) + "," + '\n'
    cmd+='MULT=' + str(s.AI_Spin) + "," + '\n'
   ...
   # then the Low Accuracy calculation at Large basis
    cmd=cfour_dat.methods[s.AI_Low]
    cmd+='MEMORY_SIZE=' + s.AI_Memory + "," + '\n'
    cmd+='CHARGE=' + str(s.AI_Charge) + "," + '\n'
    cmd+='MULT=' + str(s.AI_Spin) + "," + '\n'
...
def calc(s):
  ' Function to run an cfour calculation '
  ...
  if s.AI_JobType == 'simple':   
    ...
    print '    cfour beginning frequency calculation '
...
def scf_stability(size,s):
  ...
  if scftype == 'rscf':
    for j in range(0,len(cfour_dat.rscf)):
      # build the com file
    ...
    for j in range(0,len(cfour_dat.uscf)):
      ...
      if normal_termination(logfile,s):
        ...
        print '    cfour scf ' + size + ': calc ' + str(j+1) + ' FAILED '
        if (j+1) == len(cfour_dat.uscf):
          util.DieError('SCF','Wavefunction did not converge ',s)
  elif scftype == 'roscf':
    for j in range(0,len(cfour_dat.roscf)):
      # build the com file
      command_string = s.AI_Command['scf'][j]
      header_string = s.AI_Headers['scf']
      comfile = build_comfile(command_string,header_string,size,s)
...
def analytic_freq(job_key,size,s):
  ...
  print '    cfour analytic frequency ' + size 
  ...
  util.Run('xcfour' + ' > ' + logfile,s)
  # check for normal termination and spin contamination
  if normal_termination(logfile,s):
    print '    cfour analytic frequency ' + size + ': completed successfully'
...
#==========================================================================
# utility function used to see if cfour did its thang
#==========================================================================
def normal_termination(logfile,s):
  ...
  success=0
  for line in fc:
    if (string.find(line,'The CFour program has completed successfully') != -1):
      success=1
      break
  return success
...
#==========================================================================
# utility function to grab coordinates from a cfour log file
#==========================================================================
