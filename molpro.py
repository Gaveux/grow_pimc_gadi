#!/usr/bin/python2
#======================================
# molpro module of Grow2.0 package
#======================================

# note that data strings are defined in the module g98_dat.py
...
#==========================================================================
# a function to establish data structures for molpro methods
# called once on startup/restart
#==========================================================================

def setup(s):
  ' Function to setup for molpro calculations '
  print '    molpro setup \n' 
  # should we check here that methods exist?
  s.AI_Headers['scf'] = []
  # build a sequence of ab initio header's for scf calcs
  for i in range(0,len(molpro_dat.scf)):
    hdr='***,molpro scf stability check\n'
    hdr+='memory,' + s.AI_Memory + ',m\n'
    if s.AI_Rwf != '':
      hdr += 'file,1,' + s.AI_Rwf + '.int\n'
      hdr += 'file,2,' + s.AI_Rwf + '.wfu\n'
    hdr+='geomtyp=xyz; geometry=' + s.Molecule + '.xyz\n'
    hdr+='spherical; basis=' + s.AI_Basis + '\n'
    hdr+='charge=' + str(s.AI_Charge) + '; spin=' + str(s.AI_Spin-1) + '\n'
    hdr+='charge=' + str(s.AI_Charge) + '; spin=' + str(s.AI_Spin-1) + '\n'
  ...
  if s.AI_JobType == 'simple':
    ...
    if s.AI_Rwf != '':
      hdr += '%rwf=' + s.AI_Rwf + '\n'
    hdr+='%mem=' + s.AI_Memory
    hdr+=g98_dat.methods[s.AI_Simple]
...
#==========================================================================
# main function for running Gaussian98 jobs
#==========================================================================
...
def calc(s):
  ...
  if s.AI_JobType == 'simple':   
    ...
    print '    g98 beginning frequency calculation '
...
def frequency_calc(job_key,method,size,s):
  ...
  # interrogate the dictionary g98_dat.available
  availability = ''
  for k in g98_dat.available:
    if g98_dat.available[k].count(method) == 1:
      availability = k
      break
  ...
  if availability == 'analytic frequencies':
    ...
    util.DieError('MethodError:',' Method ' + method +' not found in g98_dat.available[]',s)
...
def numerical_freq_analytic_grad(job_key,size,s):
  ...
  # preparations
  print '    g98 numerical frequency (gradients) ' + size 
  logfile = s.Molecule + '.log'
  keyword_string = s.AI_Headers[job_key]
  ...
  for k in range(0,len(geoms)):
    ...
    # check for normal termination and spin contamination
    if normal_termination(logfile,s):
      print '    g98 gradient ' + size + ' : calc ',k
      contaminated = testspin(s.AI_Spin,logfile,s)
      if contaminated:
        util.DieError('SpinError','Correlated wavefunction is spin contaminated',s)
    else:
...
def numerical_freq_energy_only(job_key,size,s):
  ...
  # preparation 
  print '    g98 numerical frequency (energies) ' + size 
  logfile = s.Molecule + '.log'
  keyword_string = s.AI_Headers[job_key]
  # make displaced geometries, note that CARTVECS is in bohr
  util.Rmf('en.geoms')
...
#==========================================================================
# utility function to grab coordinates from a g98 log file
#==========================================================================
def coords_from_logfile(fname,s):
  ...
  for line in range(0,len(fc)):
    geom=[]
    if string.find(fc[line],'Standard orientation:') != -1:
      line+=5
      while fc[line][0:3] != ' --':
        x = fc[line].split()
