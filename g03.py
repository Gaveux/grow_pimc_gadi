#!/usr/bin/python2
#======================================
# gaussian03 module of Grow2.2 package
# modified by Meredith Jordan March 03
#======================================

# note that data strings are defined in the module g03_dat.py
...
#==========================================================================
# a function to establish data structures for g03 methods
...
def setup(s):
  ...
  for i in range(0,len(g03_dat.scf)):
  ...
  if s.AI_JobType == 'simple':
    ...
    hdr+=g03_dat.methods[s.AI_Simple]
...
def calc(s):
  ...
  if s.AI_JobType == 'simple':   
    ...
    print '    g03 beginning frequency calculation '
...
def frequency_calc(job_key,method,size,s):
  ...
  # interrogate the dictionary g03_dat.available
  availability = ''
  for k in g03_dat.available:
    if g03_dat.available[k].count(method) == 1:
      availability = k
      break
...
# util.Rmf('freq.dat')
  ...
  if availability == 'analytic frequencies':
    ...
    util.DieError('MethodError:',' Method ' + method +' not found in g03_dat.available[]',s)
...
def numerical_freq_analytic_grad(job_key,size,s):
  ...
  # preparations
  print '    g03 numerical frequency (gradients) ' + size 
  logfile = s.Molecule + '.log'
  keyword_string = s.AI_Headers[job_key]
  ...
  for k in range(0,len(geoms)):
    ...
    comfile = build_comfile(keyword_string,size,s)
    util.Run('g03 <' + comfile + ' > ' + logfile,s)
    # check for normal termination and spin contamination from Test.FChk
    fc = util.grab_contents('Test.FChk',s)
    if normal_termination(logfile,s):
      print '    g03 gradient ' + size + ' : calc ',k
      contaminated = testspin(s.AI_Spin,fc,s)
      if contaminated:
        util.DieError('SpinError','Correlated wavefunction is spin contaminated',s)
    else:
...
def numerical_freq_energy_only(job_key,size,s):
  ...
  # preparation 
  print '    g03 numerical frequency (energies) ' + size 
  logfile = s.Molecule + '.log'
  keyword_string = s.AI_Headers[job_key]
  # make displaced geometries, note that CARTVECS is in bohr
  util.Rmf('en.geoms')
...
#   util.Rmf('Test.FChk')
    ...
    comfile = build_comfile(keyword_string,size,s)
    util.Run('g03 <' + comfile + ' > ' + logfile,s)
    # check for normal termination and spin contamination from log/checkpoint file
    fc = util.grab_contents('Test.FChk',s)
    if normal_termination(logfile,s):
      print '    g03 energy ' + size + ' : calc ', k
      contaminated = testspin(s.AI_Spin,fc,s)
      if contaminated:
        util.DieError('SpinError','Correlated wavefunction is spin contaminated',s)
    else:
...
def testspin(M,fc,s):
  ...
  # now look for S**2
  for line in fc:
    if (string.find(line,'''S**2  ''') != -1):
      S2 = float(string.split(line)[2])
      break
  # compare
...
#==========================================================================
# utility function to grab coordinates from a g03 log file
#==========================================================================
