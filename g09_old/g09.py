...
#  util.Run('cp cdata_bohr add' + n + '_bohr',s)
   #append this data to the cartesian pot file
   util.Run('echo pot data' + n + ' >> pot_cart',s)
   util.Run('cat freq.dat >> pot_cart', s)
...
# util.Rmf('POT.temp')
  ...
  output = os.popen('bfinvert pot_cart IN_SYSTEM POT ' + str(s.svdtol) ,'r')
